...
class WeeklyWeightHistoryScreen extends StatefulWidget {
  static const String ID = "/weekly_weight_history_screen";
  @override
  _WeeklyWeightHistoryScreenState createState() => _WeeklyWeightHistoryScreenState();
}

class _WeeklyWeightHistoryScreenState extends State<WeeklyWeightHistoryScreen> {
  double weight = 0.0;
  final double width = 7;
  int touchedGroupIndex = -1;
  List<UserWeightModel> weeklyData = [];
  ...
  getWeeklyData(List<UserWeightModel> weeklyData, DateTime startDate){
    ...
    while(i <= DateTime.sunday){
      ...
      weeklyData.forEach((element) {
        print(DateTime(element.date.toDate().year, element.date.toDate().month, element.date.toDate().day));
        if(DateTime(element.date.toDate().year, element.date.toDate().month, element.date.toDate().day) == startDate.add(Duration(days: i))){
